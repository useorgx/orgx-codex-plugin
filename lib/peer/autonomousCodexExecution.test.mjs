...
describe('autonomous Codex execution preparation', () => {
  ...
  it('discovers MCP servers under the same plugin-disabled isolation surface', async () => {
    const calls = [];
    const servers = await discoverConfiguredMcpServers({
      async execFileImpl(command, args, options) {
        calls.push({ command, args, options });
        return { stdout: '[{"name":"orgx"}]' };
      },
    });

    assert.deepEqual(servers, ['orgx']);
