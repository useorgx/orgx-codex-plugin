...
/** Above this, report that reclaiming disk requires writer coordination. */
...
export async function writeCursor(cursorPath, cursor) {
  ...
  const temporaryPath = `${cursorPath}.${randomUUID()}.tmp`;
  await writeFile(temporaryPath, `${JSON.stringify(cursor, null, 2)}\n`, { mode: 0o600 });
  await rename(temporaryPath, cursorPath);
}

// Work in bytes: readline emits an unterminated tail, and re-encoding a sliced
// UTF-8 character changes the offset. Only newline-terminated records may move
// the delivery cursor, including when a byte ceiling cuts through a record.
async function* completeLines(spoolPath, start, end) {
  const stream = createReadStream(spoolPath, { start, end });
  ...
  try {
    for await (const chunk of stream) {
      ...
      let offset = 0;
      let newline;
      while ((newline = buffer.indexOf(10, offset)) !== -1) {
        yield { text: buffer.subarray(offset, newline).toString('utf8'), bytes: newline - offset + 1 };
        offset = newline + 1;
      }
...
export async function readSpoolBatch({
  ...
  for await (const line of completeLines(spoolPath, startOffset, end)) {
    const trimmed = line.text.trim();
    if (trimmed) {
      try {
        const record = JSON.parse(trimmed);
        if (!record || typeof record !== 'object' || Array.isArray(record)) throw new Error('Invalid hook record');
        records.push(record);
      } catch {
        malformed += 1;
      }
    }
    consumedBytes += line.bytes;
...
export async function countPendingRecords({
  ...
  return {
    pending,
    deadLetters,
    partialBytes: Math.min(remainingBytes, maxScanBytes) - consumedBytes,
    exact: !capped,
...
/**
 * Hook writers append without sharing a lock with the peer. A drained snapshot
 * cannot authorize truncation: writers can append during the upload or after a
 * fresh stat. Retain the spool until rotation coordinates with those writers.
 */
export async function rotateIfDrained({
  ...
  return { rotated: false, reclaimedBytes: 0, reason: 'writer_coordination_required' };
}
...
export async function replayHookSpoolOnce({
  ...
  // Preserve invalid records for diagnosis instead of posting a subset and
  // advancing past the malformed evidence. No cursor or spool mutation occurs.
  if (batch.malformed > 0) {
    return { status: 'blocked', reason: 'malformed_records', malformed: batch.malformed, posted: 0 };
  }
