...
export const CODEX_ISOLATION_CONFIG_ARGS = Object.freeze([
  '-c',
  'features.apps=false',
  '-c',
...
export function buildCodexProcessConfigArgs(policy, configuredServers) {
  ...
  const args = [...CODEX_ISOLATION_CONFIG_ARGS];
...
export function verifyEffectiveMcpPolicy(entries, policy) {
  ...
  for (const [server, expectedTools] of Object.entries(normalized.allowedByServer)) {
    ...
    requireHostedOAuth(entry, server);
...
export function verifyMcpReadinessStatus(
  ...
  requireHostedOAuth(required, serverName);
...
function requireHostedOAuth(entry, server) {
  const authStatus = entry.authStatus ?? entry.auth_status;
  if (authStatus !== 'oAuth') {
    fail(
      'mcp_server_unauthenticated',
      `required MCP server '${server}' must report the current Codex OAuth success status`
    );
  }
}
