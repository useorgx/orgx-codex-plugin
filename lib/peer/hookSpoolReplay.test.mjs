...
describe('readSpoolBatch', () => {
  ...
  it('retries a partial trailing record after its writer finishes', async () => {
    const second = line(1);
    await writeFile(spoolPath, line(0) + second.slice(0, -5));
    const first = await readSpoolBatch({ spoolPath });
    assert.deepEqual(first.records.map((r) => r.seq), [0]);
    assert.equal(first.nextOffset, Buffer.byteLength(line(0)));
    assert.equal(first.drained, false);
    assert.equal(first.malformed, 0);
    await appendFile(spoolPath, second.slice(-5));
    const next = await readSpoolBatch({ spoolPath, startOffset: first.nextOffset });
    assert.deepEqual(next.records.map((r) => r.seq), [1]);
    assert.equal(next.drained, true);
  });

  it('keeps the byte cursor before a UTF-8 record cut by the batch ceiling', async () => {
    const firstLine = JSON.stringify({ text: 'Résumé 中文' }) + '\r\n';
    const secondLine = JSON.stringify({ text: '日本語' }) + '\n';
    await writeFile(spoolPath, firstLine + secondLine);
    const first = await readSpoolBatch({ spoolPath, maxBytes: Buffer.byteLength(firstLine) + 11 });
    assert.deepEqual(first.records, [{ text: 'Résumé 中文' }]);
    assert.equal(first.nextOffset, Buffer.byteLength(firstLine));
    const second = await readSpoolBatch({ spoolPath, startOffset: first.nextOffset });
    assert.deepEqual(second.records, [{ text: '日本語' }]);
...
describe('replayHookSpoolOnce', () => {
  ...
  it('preserves records appended during an upload of an oversized spool', async () => {
    await seedSpool(3);
    const seen = [];
    await replayHookSpoolOnce({
      spoolPath, cursorPath, rotateBytes: 1,
      postImpl: async (records) => {
        seen.push(...records.map((r) => r.seq));
        await appendFile(spoolPath, line(99));
      },
    });
    await replayHookSpoolOnce({
      spoolPath, cursorPath, rotateBytes: 1,
  ...
  it('keeps malformed evidence and the cursor intact for diagnosis', async () => {
    const original = line(0) + 'broken record\n' + line(1);
    await writeFile(spoolPath, original);
    let called = false;
    const result = await replayHookSpoolOnce({
      spoolPath, cursorPath,
      postImpl: async () => { called = true; },
    });
    assert.equal(result.status, 'blocked');
    assert.equal(result.reason, 'malformed_records');
    assert.equal(called, false);
    assert.equal((await readCursor(cursorPath, await stat(spoolPath))).byteOffset, 0);
    assert.equal(await readFile(spoolPath, 'utf8'), original);
  });
...
describe('rotateIfDrained', () => {
  it('retains even a drained snapshot while hook writers can append', async () => {
    await seedSpool(100);
    const { size } = await stat(spoolPath);
    ...
    assert.equal(result.reason, 'writer_coordination_required');
    assert.equal((await stat(spoolPath)).size, size);
  });
