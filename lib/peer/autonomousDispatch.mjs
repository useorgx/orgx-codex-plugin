...
export class AutonomousDispatchGuard {
  constructor(driver, opts = {}) {
    ...
    this.now = opts.now;
    // This callback is a runner-owned trust boundary. It must never be
    // sourced from the remote task or model output.
    this.verifyAutonomousVerifier = opts.verifyAutonomousVerifier;
  }
  ...
  async *dispatch(task, context) {
    ...
    try {
      ...
      const validator = new AutonomousTerminalEvidenceValidator(
        autonomousAuthority,
        {
          runId,
          verifyVerifier: this.verifyAutonomousVerifier,
        }
      );
      for await (const message of this.driver.dispatch(task, {
        ...context,
        ...
        const decision = validator.evaluate(message);
        yield terminalGateStep(runId, decision);
        yield {
          ...message,
          outcome_kind: decision.outcomeKind,
        };
        return;
      }
    } catch (error) {
