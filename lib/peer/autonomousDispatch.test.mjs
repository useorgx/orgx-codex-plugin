...
describe('autonomous dispatch contract', () => {
  ...
  it('accepts the one-hour control-plane lease ceiling', async () => {
    const fixture = await buildFixture({
      issuedAt: '2026-07-17T11:30:00.000Z',
      expiresAt: '2026-07-17T12:30:00.000Z',
      deadline: '2026-07-17T12:15:00.000Z',
    });
  ...
  it('rejects a capability lease longer than the one-hour control-plane ceiling', async () => {
    const fixture = await buildFixture({
      issuedAt: '2026-07-17T11:30:00.000Z',
      expiresAt: '2026-07-17T12:30:00.001Z',
      deadline: '2026-07-17T12:15:00.000Z',
    });
    ...
    await assert.rejects(
      ...
      (error) =>
        error instanceof AutonomousDispatchContractError &&
        error.code === 'capability_lease_ttl_exceeded'
    );
  });
});
...
describe('AutonomousDispatchGuard', () => {
  ...
  it('passes only validated authority to the delegate', async () => {
    ...
    assert.equal(messages.at(-1).kind, 'task.completed');
    assert.equal(messages.at(-1).outcome_kind, 'blocked');
    assert.match(
      messages.at(-2).step.summary,
  ...
  it('rejects stale authority before dispatch with an expired-at-observation receipt code', async () => {
    const fixture = await buildFixture({
      issuedAt: '2026-07-17T10:00:00.000Z',
      expiresAt: '2026-07-17T11:00:00.000Z',
      deadline: '2026-07-17T10:30:00.000Z',
    ...
    assert.match(messages[0].reason, /^capability_lease_expired:/);
    assert.equal(delegate.dispatches.length, 0);
  });
});
...
async function buildFixture(opts = {}) {
  ...
  const expiresAt = opts.expiresAt ?? '2026-07-17T12:55:00.000Z';
